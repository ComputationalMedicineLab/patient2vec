...
SELECT * FROM
  EXTERNAL '<PATH_TO_THE_PROJECT>/patient2vec/breast_cancer/data/intermediate/rxcui_atc_map.csv'
USING
(
 DELIMITER ','
...
-------------------------------------------------------------------------------
-- TABLE p2v_breast_cancer_meds_atc_map
-------------------------------------------------------------------------------
...
--  - atc_class
-------------------------------------------------------------------------------
CREATE TABLE p2v_breast_cancer_meds_atc_map AS
(
  ...
  WHERE lower(medex.generic_name) in
  	(SELECT DISTINCT code FROM p2v_breast_cancer_all_patients_events WHERE type = 'med')
)

-- breast_cancer/data/intermediate/generic_name_atc_map.csv
SELECT * FROM p2v_breast_cancer_meds_atc_map ORDER BY generic_name;

-- breast_cancer/data/intermediate/icd_phewas_map.csv
