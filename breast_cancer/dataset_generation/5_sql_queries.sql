-------------------------------------------------------------------------------
-- TABLE p2v_breast_cancer_patients_outcome
-------------------------------------------------------------------------------
...
--  - t_group (dataset - train/test/validation)
-------------------------------------------------------------------------------

CREATE TABLE p2v_breast_cancer_patients_outcome (
...
INSERT INTO p2v_breast_cancer_patients_outcome
SELECT * FROM
  EXTERNAL '<PATH_TO_THE_PROJECT>/patient2vec/breast_cancer/data/intermediate/patient_list_split.csv'
USING
(
 DELIMITER ','
...
-------------------------------------------------------------------------------
-- TABLE p2v_breast_cancer_all_patients_events
-------------------------------------------------------------------------------
-- Contains all codes (ICD-9/medication/lab) for patients selected for
-- the experiment (table p2v_breast_cancer_patients_outcome) before cutoff date.
-- Columns:
--  - ind_seq
--  - entry_date
...
CREATE TABLE p2v_breast_cancer_all_patients_events AS
(
  (
    ...
    (
      SELECT codes.ind_seq, codes.entry_date, codes.code, outcome.cutoff_date FROM
        (
          ...
          WHERE ind_seq IN (SELECT DISTINCT ind_seq FROM p2v_breast_cancer_patients_outcome)
          ORDER BY ind_seq, entry_date
        ) as codes
      LEFT JOIN
        (SELECT ind_seq, cutoff_date FROM p2v_breast_cancer_patients_outcome) AS outcome
...
-- breast_cancer/data/raw/breast_cancer_patients_codes.csv
SELECT * FROM p2v_breast_cancer_all_patients_events;
